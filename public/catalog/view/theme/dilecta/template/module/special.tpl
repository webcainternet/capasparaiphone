<?php $id_module = rand(1,999); ?>
		...
		<section id="special_<?php echo $id_module; ?>" class="box">
			
			<!-- Title -->
			
			<h2 class="box-title"><?php echo $heading_title; ?></h2>
			
		  		<?php if(!($this->config->get('product_scroll_specials') == '0' && $this->config->get('general_status') == '1')) { ?>
				<!-- Pagination -->
				
				<div class="pagination-product clearfix">
				
					<a href="#" class="prev-products" id="module_special_<?php echo $id_module; ?>_prev"></a>
					<a href="#" class="next-products" id="module_special_<?php echo $id_module; ?>_next"></a>
				
				</div>
								
				<script type="text/javascript">
					jQuery(function($){
						paginacja('module_special_<?php echo $id_module; ?>');
					});
				</script>
				<?php } ?> 
				...
				<div class="overflow-products clearfix" id="module_special_<?php echo $id_module; ?>">
					...
					<!-- Product -->
						...
						<div>
							...
							<div class="left">
								...
								<div class="image"><?php if ($product['price'] && $product['special']) { $sale = 'Desconto'; if(isset($text_sale)) { if($text_sale != 'text_sale') { $sale = $text_sale; } } echo '<div class="sale" style="text-transform:uppercase">'.$sale.'</div>'; } ?><a href="<?php echo $product['href']; ?>"><img src="<?php echo $product['thumb']; ?>" alt="<?php echo $product['name']; ?>" /></a></div>
								<?php } ?>
